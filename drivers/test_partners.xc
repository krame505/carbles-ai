...
# endif
  for (n = 0; n < games; n++) {
    ...
    with_arena ar {
      ...
      for (unsigned i = 0; i < numPlayers; i++) {
        trialPlayers[i] = getPlayer(ar, players[ps[i]], numPlayers * 2);
        trialPlayers[i + numPlayers] = getPlayer(ar, players[ps[i]], numPlayers * 2);
      }

      winner = ps[playQuietGame(numPlayers * 2, true, openHands, trialPlayers)];
    }
